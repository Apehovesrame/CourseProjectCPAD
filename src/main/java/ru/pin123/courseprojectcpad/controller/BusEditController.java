...
public class BusEditController {
    ...
    private void handleOk() {
        if (isInputValid()) {
            ...
            bus.setLicensePlate(tfLicensePlate.getText());
            bus.setSeatCapacity(Integer.parseInt(tfSeatCapacity.getText()));
    ...
    // Валидация данных (Задание из Лабораторной №7)
    private boolean isInputValid() {
        String errorMessage = "";

        if (tfModel.getText() == null || tfModel.getText().trim().isEmpty()) {
            errorMessage += "Не указана марка/модель автобуса!\n";
        }
        if (tfLicensePlate.getText() == null || tfLicensePlate.getText().trim().isEmpty()) {
            errorMessage += "Не указан госномер!\n";
        }
